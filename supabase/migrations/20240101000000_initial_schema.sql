...
create policy "Coaches view own dojos" on dojos for select using (auth.uid() = coach_id);
create policy "Coaches insert own dojos" on dojos for insert with check (auth.uid() = coach_id);
create policy "Coaches update own dojos" on dojos for update using (auth.uid() = coach_id);
...
create policy "Coaches manage their students" on students using (
  exists (select 1 from dojos where dojos.id = students.dojo_id and dojos.coach_id = auth.uid())
);
...
create policy "View categories" on categories for select using (true);
-- Organizers manage.
create policy "Organizers manage categories" on categories using (
  exists (select 1 from events where events.id = categories.event_id and events.organizer_id = auth.uid())
);
create policy "View event days" on event_days for select using (true);
create policy "Organizers manage event days" on event_days using (
  exists (select 1 from events where events.id = event_days.event_id and events.organizer_id = auth.uid())
);
...
create policy "Organizers view entries for their events" on entries for select using (
  exists (select 1 from events where events.id = entries.event_id and events.organizer_id = auth.uid())
);
...
create policy "Coaches apply" on event_applications for insert with check (auth.uid() = coach_id);
create policy "Coaches view own applications" on event_applications for select using (auth.uid() = coach_id);
